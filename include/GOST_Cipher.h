...
#ifndef CURSE_PROJECT_GOST_CIPHER_H
#define CURSE_PROJECT_GOST_CIPHER_H
...
void GOST_split_256bits_to_32bits(uint8_t *key256b, uint32_t *keys32b);

void GOST_split_64bits_to_32bits(uint64_t block64b, uint32_t *block32b_1, uint32_t *block32b_2);

void GOST_split_64bits_to_8bits(uint64_t block64b, uint8_t *blocks8b);

void GOST_split_32bits_to_8bits(uint32_t block32b, uint8_t *blocks4b);

uint64_t GOST_join_32bits_to_64bits(uint32_t block32b_1, uint32_t block32b_2);

uint64_t GOST_join_8bits_to_64bits(uint8_t *blocks8b);

uint32_t GOST_join_4bits_to_32bits(uint8_t *blocks4b);
...
size_t GOST_28147(uint8_t *to, uint8_t mode, uint8_t *key256b, uint8_t *from, size_t length) {
    ...
    GOST_split_256bits_to_32bits(key256b, keys32b);

    for (size_t i = 0; i < length; i += 8) {
        GOST_split_64bits_to_32bits(
                GOST_join_8bits_to_64bits(from + i),
                &N1, &N2
        );
        feistel_cipher(mode, &N1, &N2, keys32b);
        GOST_split_64bits_to_8bits(
                GOST_join_32bits_to_64bits(N1, N2),
                (to + i)
        );
    }
...
uint32_t substitution_table(uint32_t block32b, uint8_t sbox_row) {
    ...
    GOST_split_32bits_to_8bits(block32b, blocks4bits);
    substitution_table_by_4bits(blocks4bits, sbox_row);
    return GOST_join_4bits_to_32bits(blocks4bits);
}
...
void substitution_table_by_4bits(uint8_t *blocks4b, uint8_t sbox_row) {
    ...
    for (uint8_t i = 0; i < 4; ++i) {
        ...
        block4b_1 = GOST_Sbox[sbox_row][blocks4b[i] & 0x0F];

        // 10101100 >> 4 = 00001010
        // [example get from table] 1010 -> 0111
        block4b_2 = GOST_Sbox[sbox_row][blocks4b[i] >> 4];
...
void GOST_split_32bits_to_8bits(uint32_t block32b, uint8_t *blocks8b) {
    for (uint8_t i = 0; i < 4; ++i) {
...
#endif //CURSE_PROJECT_GOST_CIPHER_H